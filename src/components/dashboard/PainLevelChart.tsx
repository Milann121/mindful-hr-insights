...
const PainLevelChart = () => {
  ...
  const [data, setData] = useState([{
    name: t('dashboard.painLevels.low'),
    value: 0,
    color: '#10B981'
  }, {
    name: t('dashboard.painLevels.moderate'),
    value: 0,
    color: '#F59E0B'
  }, {
    name: t('dashboard.painLevels.high'),
    value: 0,
    color: '#EF4444'
  }, {
    name: t('dashboard.painLevels.noData'),
    value: 0,
    color: '#6B7280'
  }]);
  ...
  useEffect(() => {
    const fetchPainLevelData = async () => {
      try {
        ...
        const {
          data: programData,
          error: programError
        } = await supabase.from('user_program_tracking').select('initial_pain_level, program_started_at, program_ended_at, program_status').in('b2b_employee_id', employeeIds).lte('program_started_at', end.toISOString()).or(`program_ended_at.is.null,program_ended_at.gte.${start.toISOString()}`).neq('program_status', 'deleted');
        if (programError) {
        ...
        if (total > 0) {
          setData([{
            ...
            name: t('dashboard.painLevels.moderate'),
            value: Math.round(moderateCount / total * 100),
            color: '#F59E0B'
          }, {
            name: t('dashboard.painLevels.high'),
            ...
            name: t('dashboard.painLevels.noData'),
            value: Math.round(noDataCount / total * 100),
            color: '#6B7280'
          }]);
        } else {
          setData([{
    ...
    const channel = supabase.channel('pain-level-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
  ...
  return <Card className="col-span-1 px-0 my-[21px] py-px">
      ...
      <CardContent className="py-0 my-[46px] px-0">
        {loading ? <div className="flex items-center justify-center h-[300px]">
            <div className="text-muted-foreground">Loading...</div>
          </div> : <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={filteredData} cx="50%" cy="50%" labelLine={false} outerRadius={100} fill="#8884d8" dataKey="value" label={({
            value
          }) => `${value}%`}>
                {filteredData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
              </Pie>
              <Tooltip formatter={value => [`${value}%`, 'Percentage']} />
              <Legend verticalAlign="bottom" height={24} layout="horizontal" align="center" wrapperStyle={{
            paddingTop: '16px'
