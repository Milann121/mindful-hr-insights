...
const TopIssuesChart = () => {
  ...
  const [jobProperties, setJobProperties] = useState<Array<{
    id: string;
    name: string;
  }>>([]);
  ...
  useEffect(() => {
    const fetchFiltersData = async () => {
      try {
        ...
        } = await supabase.from('company_departments').select('id, department_name');
        if (departmentsData) {
        ...
        const {
          data: jobPropertiesData
        } = await supabase.from('job_properties').select('id, property_name');
        if (jobPropertiesData) {
          setJobProperties(jobPropertiesData.map(jp => ({
            id: jp.id,
            name: jp.property_name
          })));
        }
      } catch (error) {
      ...
      try {
        ...
        if (viewBy === 'overall') {
          ...
          const {
            data: fetchedData,
            error: programError
          } = await supabase.from('user_program_tracking').select(`
              pain_area,
              b2b_employee_id,
              program_started_at,
              program_ended_at,
              program_status
            `).in('b2b_employee_id', employeeIds).lte('program_started_at', end.toISOString()).or(`program_ended_at.is.null,program_ended_at.gte.${start.toISOString()}`).neq('program_status', 'deleted');
          if (programError) {
          ...
          if (fetchedData) {
            ...
            const {
              data: profilesData
            } = await supabase.from('user_profiles').select('user_id, department_id').in('user_id', userIds).eq('department_id', selectedDepartment);
          ...
          if (fetchedData) {
            ...
            const {
              data: profilesData
            } = await supabase.from('user_profiles').select('user_id, department_id').in('user_id', userIds);
            if (profilesData) {
              const departmentIds = profilesData.map(p => p.department_id).filter(Boolean);
              const {
                data: jobPropsData
              } = await supabase.from('department_job_properties').select('department_id').in('department_id', departmentIds).eq('job_property_id', selectedJobProperty);
              const validDepartmentIds = jobPropsData?.map(jp => jp.department_id) || [];
              const filteredUserIds = profilesData.filter(p => validDepartmentIds.includes(p.department_id)).map(p => p.user_id);
              programData = fetchedData.filter(p => filteredUserIds.includes(p.user_id));
            }
          }
        }
  ...
  const CustomLegend = () => {
    ...
    return <div className="mt-4 block sm:hidden">
        <div className="grid grid-cols-1 gap-2">
          {data.map((entry, index) => <div key={index} className="flex items-center gap-2 py-0 my-[2px] px-0 mx-[37px]">
  ...
  return <Card className="col-span-1 lg:col-span-1">
      <CardHeader>
        ...
        <div className="flex flex-wrap items-center gap-2 mt-4">
          {/* Overall button */}
          <Button variant={viewBy === 'overall' ? 'default' : 'outline'} onClick={() => {
          setViewBy('overall');
          ...
          <Select value={viewBy === 'department' ? selectedDepartment : ''} onValueChange={value => {
          ...
          <Select value={viewBy === 'jobProperties' ? selectedJobProperty : ''} onValueChange={value => {
          ...
          setSelectedDepartment('');
            ...
            <SelectContent>
              {jobProperties.map(jobProp => <SelectItem key={jobProp.id} value={jobProp.id}>
      ...
      <CardContent className="my-0 py-[15px]">
        {loading ? <div className="h-[300px] flex items-center justify-center">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={data} barCategoryGap="20%">
                ...
                <XAxis dataKey="name" className="hidden sm:block" fontSize={12} angle={-45} textAnchor="end" height={60} />
                <YAxis />
                <Tooltip />
